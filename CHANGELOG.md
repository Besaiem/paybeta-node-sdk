...
- package renamed from @paybeta/node to @paybetaby/node-sdk. Default baseUrl changed from https://api.paybeta.com to https://api.usepaybeta.com. list() return shapes changed per resource (bare arrays for payments/transactions/disputes; { escrows, total, limit, offset } for escrows) instead of a generic { data, total, limit, offset } envelope. webhooks.constructEvent() now requires a third `timestamp` argument and verifies HMAC-SHA256 instead of SHA512. Several dispute and escrow enum values changed to match the API's actual accepted values.
